...
async function createRental(vehicleId, renterId, unlockCode, startDateTime, endDateTime, totalAmount, executor = null) {
...
async function hasOverlappingRental(vehicleId, startDateTime, endDateTime, executor = null) {
  const pool = executor || db.getPool();
  const { rows } = await pool.query(
    `SELECT 1 FROM rentals WHERE vehicle_id = $1 AND start_time < $3 AND end_time > $2 LIMIT 1`,
    [vehicleId, startDateTime, endDateTime]
  );
  return rows.length > 0;
}

async function findRentalsByUserId(userId, executor = null) {
  const pool = executor || db.getPool();
  const { rows } = await pool.query(
    `SELECT ${RENTAL_SELECT} FROM rentals r JOIN vehicles v ON v.id = r.vehicle_id WHERE r.renter_id = $1 ORDER BY r.start_time DESC`,
    [userId]
  );
  return rows;
}

async function findRentalByIdForUser(rentalId, userId, executor = null) {
  const pool = executor || db.getPool();
  const { rows } = await pool.query(
    `SELECT id, renter_id AS "renterId", start_time AS "startDateTime", end_time AS "endDateTime", returned_at AS "returnedAt"
     FROM rentals WHERE id = $1 AND renter_id = $2`,
    [rentalId, userId]
  );
  return rows[0] ?? null;
}

async function returnRentalById(rentalId, userId, executor = null) {
  const pool = executor || db.getPool();
  const { rows } = await pool.query(
    `UPDATE rentals r
     ...
     FROM vehicles v
     WHERE r.id = $1 AND r.renter_id = $2 AND v.id = r.vehicle_id
...
async function findRentalsByVehicleOwnerId(userId, executor = null) {
  const pool = executor || db.getPool();
  const { rows } = await pool.query(
    `SELECT ${RENTAL_SELECT} FROM rentals r
     JOIN vehicles v ON v.id = r.vehicle_id
     JOIN ownerships o ON o.vehicle_id = v.id
     WHERE o.owner_id = $1 ORDER BY r.start_time DESC`,
    [userId]
  );
  return rows;
}
